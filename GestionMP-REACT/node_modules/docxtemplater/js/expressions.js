...
function getIndex(scope, context) {
  var index = null;
  for (var i = context.scopePathItem.length - 1; i >= 0; i--) {
...
function _getObjectIdentifiers(x) {
  ...
  if (x.body) {
    for (var _i4 = 0, _x$body2 = x.body; _i4 < _x$body2.length; _i4++) {
      var y = _x$body2[_i4];
      _getObjectIdentifiers(y, scope);
    }
    return scope;
  }
  if (x.type === "CallExpression") {
    ...
    if (x.arguments) {
      for (var _i6 = 0, _x$arguments2 = x.arguments; _i6 < _x$arguments2.length; _i6++) {
        var _y = _x$arguments2[_i6];
        _getObjectIdentifiers(_y, scope);
      }
    }
  }
  if (x.ast) {
...
function configuredParser() {
  var config = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  return function parser(tag) {
    if (typeof tag !== "string") {
      throw new Error("The angular parser was used incorrectly, please refer to the documentation of docxtemplater.");
    }
    ...
    var expr = expressions.compile(tag, _objectSpread({
     ...
     */
    var lastBody = expr.ast.body[expr.ast.body.length - 1];
    var isAssignment = lastBody && lastBody.expression.type === "AssignmentExpression";
    return {
        ...
        var px = new Proxy({}, {
          get: function get(target, name) {
            ...
            if (config.evaluateIdentifier) {
              var fnResult = config.evaluateIdentifier(name, scope, scopeList, context);
              if (fnResult != null) {
                return fnResult;
              }
            }
            ...
            if (hasOwnProperty.call(scope, name) && scope[name] != null) {
              var property = scope[name];
              return typeof property === "function" ? property.bind(scope) : property;
            }
            for (var i = scopeList.length - 1; i >= 0; i--) {
              var s = scopeList[i];
              if (s != null && hasOwnProperty.call(s, name) && s[name] != null) {
                var _property = s[name];
                return typeof _property === "function" ? _property.bind(s) : _property;
              }
            }
            return null;
          },
          ...
          set: function set(target, name, value) {
            ...
            if (config.setIdentifier) {
              var fnResult = config.setIdentifier(name, value, scope, scopeList, context);
              if (fnResult != null) {
                return true;
              }
            }
      ...
      getIdentifiers: function getIdentifiers() {
        return uniq(_getIdentifiers(expr));
      },
...
exportedValue.configure = function (config) {
  return configuredParser(config);
};
