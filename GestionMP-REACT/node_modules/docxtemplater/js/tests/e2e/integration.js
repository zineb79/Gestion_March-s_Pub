...
describe("Simple templating", function () {
  ...
  it("should replace custom properties text", function () {
    ...
    var doc = createDocV4("properties.docx", {
      modules: [module]
    });
...
describe("Regression", function () {
  ...
  it("should fail with specific error message with xlsx file", function () {
    /*
    ...
    expectToThrow(function () {
      return createDocV4("simple.xlsx");
    }, Error, expectedError);
  });
});
...
describe("Dash Loop", function () {
  it("should work on simple table -> w:tr", function () {
    var doc = createDocV4("tag-dash-loop.docx").render({
      os: [{
        type: "linux",
        price: "0",
        reference: "Ubuntu10"
  ...
  it("should work on simple table -> w:table", function () {
    var doc = createDocV4("tag-dash-loop-table.docx").render({
      os: [{
        type: "linux",
        price: "0",
        reference: "Ubuntu10"
  ...
  it("should work on simple list -> w:p", function () {
    var doc = createDocV4("tag-dash-loop-list.docx").render({
      os: [{
        type: "linux",
        price: "0",
        reference: "Ubuntu10"
...
describe("ParagraphLoop", function () {
  ...
  it("should not fail when having paragraph in paragraph", function () {
    ...
    var filePath = "";
    var doc = createDocV4("regression-par-in-par.docx", {
      modules: [{
        name: "MyModule",
  ...
  it("should fail with errors from header and footer", function () {
    ...
    expectToThrow(function () {
      return createDocV4("errors-footer-and-header.docx", {
        paragraphLoop: true,
        errorLogging: false,
        parser: expressionParser
      });
  ...
  it("should fail properly when having lexed + postparsed errors", function () {
    ...
    expectToThrow(function () {
      return createDocV4("multi-errors.docx", {
        paragraphLoop: true,
        errorLogging: false,
        parser: expressionParser
      });
  ...
  it("should fail when placing paragraph loop inside normal loop", function () {
    ...
    expectToThrow(function () {
      return createDocV4("paragraph-loop-error.docx", {
        errorLogging: false
      });
    }, Errors.XTTemplateError, expectedError);
  });
...
describe("Prefixes", function () {
  it("should be possible to change the prefix of the loop module", function () {
    var doc = makeDocxV4("<w:t>{##tables}{user}{/tables}</w:t>", {
      modules: [{
  ...
  it("should be possible to change the prefix of the loop module to a regexp", function () {
    var doc = makeDocxV4("<w:t>{##tables}{user}{/tables}{#tables}{user}{/tables}</w:t>", {
      modules: [{
        optionsTransformer: function optionsTransformer(options, doc) {
          for (var _i4 = 0, _doc$modules4 = doc.modules; _i4 < _doc$modules4.length; _i4++) {
            var module = _doc$modules4[_i4];
            if (module.name === "LoopModule") {
              module.prefix.start = /^##?(.*)$/;
            }
          }
  ...
  it("should be possible to use FOR and ENDFOR for the prefix of the loop module", function () {
    var doc = makeDocxV4("<w:t>{FOR tables}{user}{ENDFOR tables}{FOR tables}{user}{ENDFOR}</w:t>", {
      modules: [{
        optionsTransformer: function optionsTransformer(options, doc) {
          for (var _i6 = 0, _doc$modules6 = doc.modules; _i6 < _doc$modules6.length; _i6++) {
            var module = _doc$modules6[_i6];
            if (module.name === "LoopModule") {
  ...
  it("should be possible to change the prefix of the raw xml module to a regexp", function () {
    var doc = makeDocxV4("<w:p><w:t>{!!raw}</w:t></w:p>", {
      modules: [{
        optionsTransformer: function optionsTransformer(options, doc) {
          for (var _i8 = 0, _doc$modules8 = doc.modules; _i8 < _doc$modules8.length; _i8++) {
            var module = _doc$modules8[_i8];
  ...
  it("should be possible to change the prefix of the raw xml module to a function", function () {
    var content = "<w:p><w:t>{raw}</w:t></w:p><w:p><w:t> {text}</w:t></w:p>";
    var scope = {
      raw: "<w:p><w:t>HoHo</w:t></w:p>",
      text: "Huhu"
    };
    var doc = makeDocxV4(content, {
      modules: [{
        optionsTransformer: function optionsTransformer(options, doc) {
          for (var _i0 = 0, _doc$modules0 = doc.modules; _i0 < _doc$modules0.length; _i0++) {
            var module = _doc$modules0[_i0];
            if (module.name === "RawXmlModule") {
    ...
    doc.render(scope);
    expect(doc.zip.file("word/document.xml").asText()).to.be.equal('<w:p><w:t>HoHo</w:t></w:p><w:p><w:t xml:space="preserve"> Huhu</w:t></w:p>');
  });
});
...
describe("Smart arts", function () {
  it("should work with smart-art on pptx files", function () {
    var doc = createDocV4("smart-art.pptx");
    doc.render({
      user: "John",
      i: 33,
      j: "J"
    ...
    shouldBeSame({
      doc: doc,
      expectedName: "expected-smart-art.pptx"
    });
  });
  it("should work with smart-art on docx files", function () {
    var doc = createDocV4("smart-art.docx");
    doc.render({
      name: "Jack",
      user: "John",
      i: 33,
    ...
    shouldBeSame({
      doc: doc,
      expectedName: "expected-smart-art.docx"
    });
  });
});
describe("Add module to change justify alignment", function () {
  it("should be possible to add w:doNotExpandShiftReturn to word/settings.xml", function () {
    var doc = createDocV4("justify.docx", {
      linebreaks: true,
      paragraphLoop: true,
      modules: [{
        name: "AddDoNotExpandShiftReturn",
        optionsTransformer: function optionsTransformer(options, docxtemplater) {
          docxtemplater.fileTypeConfig.tagsXmlLexedArray.push("w:compat", "w:settings", "w:doNotExpandShiftReturn");
          return options;
        },
        preparse: function preparse(xml, options) {
          var filePath = options.filePath;
          if (filePath === "word/settings.xml") {
            var addedDoNotExpandShiftReturn = false;
            var added = [];
            var addIndex = -1;
            ...
            if (addIndex !== -1) {
              xml.splice.apply(xml, [addIndex, 0].concat(added));
            }
          }
          return xml;
        }
    ...
    shouldBeSame({
      doc: doc,
      expectedName: "expected-tag-justified.docx"
    });
  });
});
describe("StripInvalidXml", function () {
  it("should be possible to strip invalid xml chars", function () {
    return this.render({
      name: "tag-example.docx",
      data: {
        first_name: "John" + String.fromCharCode(28) + " Doe"
      },
      options: {
        paragraphLoop: true,
        errorLogging: false,
  ...
  it("should be possible to strip multiple invalid xml chars in same string", function () {
    ...
    return this.render({
      name: "tag-example.docx",
      data: {
        first_name: "John" + String.fromCharCode(28) + " Doe" + String.fromCharCode(28)
      },
      options: {
        paragraphLoop: true,
        errorLogging: false,
...
describe("OptionsTransformer", function () {
  it("should be possible to change delimiter without side effects", function () {
    /*
     * This was fixed since v3.55.0
     * Previously, changing options in optionsTransformer would change the default for everyone.
     */
    var sideEffectDoc = createDocV4("gt-delimiters.docx", {
      modules: [{
        name: "TestModule",
...
describe("Syntax.allowUnbalancedLoops option", function () {
  it("should work with unbalanced loop with allowUnbalancedLoops option", function () {
    var doc = createDocV4("table-unbalanced-loop.docx", {
      syntax: {
  ...
  it("should work with unbalanced loop with allowUnbalancedLoops option (2)", function () {
    var doc = createDocV4("table-unbalanced-loop-2.docx", {
      syntax: {
        allowUnbalancedLoops: true
      }
    });
...
describe("Get Tags", function () {
  it("should be possible to get the tags", function () {
    var doc = createDocV4("tag-example.docx", {
      syntax: {
        allowUnbalancedLoops: true
      }
  ...
  it("should be possible to deactivate all modules and keep the template as is", function () {
    var doc = createDocV4("loop-valid.docx", {
      modules: [{
        name: "DeactivateAllModules",
        priority: 100,
        optionsTransformer: function optionsTransformer(options, docxtemplater) {
          docxtemplater.modules = docxtemplater.modules.filter(function (mod) {
            return mod.name === "Render" || mod.name === "Common";
          });
          return options;
        }
      }],
